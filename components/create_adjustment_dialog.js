...
function createAdjustmentDialog()
{
	...
	function createMainButtonGroup(parent)
	{
		...
		var cancel = UI.button(parent,"All Done",function(){result = false;w.close();});
		var submit = UI.button(parent,"Export This Piece",function(){
			...
			if(!g_pieceSelect.listbox.selection || !g_rosterSelect.listbox.selection)
			{
				alert("Please select the piece name and player first.");
				return;
			}
			var saveName = g_pieceSelect.listbox.selection.text + "_" + g_rosterSelect.listbox.selection.text + ".pdf";
			log.l("Re-exporting piece: " + saveName);
			exportPiece(saveName);
