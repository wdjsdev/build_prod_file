...
function manuallyPopulateOrderData ()
{
	...
	var sizeListbox = UI.listbox( sizeGroup, undefined, garSizes, {
		multiselect: true,
	} );
	...
	if ( result.valid )
	{
		...
		newGarment.code = curGarmentLayer.name
			.match( pat )[ 1 ]
			.replace( /-/g, "_" )
			.replace( /_/, "-" );
		newGarment.mid = newGarment.code;
		newGarment.designNumber = "no_design_number";
		newGarment.prepressDoc = app.activeDocument;
